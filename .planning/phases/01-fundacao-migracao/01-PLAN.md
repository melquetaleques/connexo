...
- [x] **Task 1.1.1**: Criar `LoginPage.tsx` em `app/web/src/pages` baseada em `src/auth.jsx` e `tema_connexo/login/code.html`.
  - Aplicar design Sovereign Gilded.
  - Integrar com `POST /api/auth/login`.
- [x] **Task 1.1.2**: Criar `RegisterPage.tsx` com o wizard de 3 etapas (Escritório, Administrador, Plano).
  - Integrar com `POST /api/auth/register`.
- [x] **Task 1.1.3**: Refatorar `App.tsx` para incluir um componente `Layout` persistente.
...
- [x] **Task 1.2.1**: Implementar `ClientsPage.tsx` com listagem de clientes.
  - Consumir `GET /api/adv/clients`.
  - Usar componente de Tabela moderna.
- [x] **Task 1.2.2**: Criar Modal de cadastro de novo cliente.
  - Integrar com `POST /api/adv/clients`.
- [x] **Task 1.2.3**: Implementar `ProcessPage.tsx` (ou similar) para visualização de processos de um cliente.
  - Integrar com `GET /api/adv/processes`.

### Plan 1.3: Sincronização de Tipos e Modelos
- [x] **Task 1.3.1**: Criar diretório `app/web/src/types` e definir interfaces TS para `User`, `Client`, `Process`, `Accountant`.
- [x] **Task 1.3.2**: Configurar cliente Axios global em `app/web/src/lib/api.ts` com interceptors de Auth.
- [x] **Task 1.3.3**: Substituir referências ao mock `connexo-data.ts` no `DashboardPage.tsx` por chamadas à API.
